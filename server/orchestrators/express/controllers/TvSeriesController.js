...
class TvSeriesController {
    ...
    static async getOne(req, res) {
        ...
        if (series) {
            ...
            axios.get(`http://localhost:4010/tvSeries/${req.params.id}`)
                .then((tvSeries) => {
    ...
    static add(req, res) {
        axios.post('http://localhost:4010/tvSeries', {
    ...
    static update(req, res) {
        axios.put(`http://localhost:4010/tvSeries/${req.params.id}`, {
            title: req.body.title,
            overview: req.body.overview,
            poster_path: req.body.poster_path,
            popularity: req.body.popularity,
    ...
    static delete(req, res) {
        axios.delete(`http://localhost:4010/tvSeries/${req.params.id}`)
            .then(async (tvSeries) => {
                const currentTVSeries = await redis.get('tvSeries')

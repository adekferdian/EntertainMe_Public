...
class MovieController {
    ...
    static async getOne (req, res) {
        ...
        if(movies){
            ...
            axios.get(`http://localhost:4009/movies/${req.params.id}`)
            .then( (movie) => {
    ...
    static add (req, res) {
        axios.post('http://localhost:4009/movies', {
    ...
    static update( req, res) {
        axios.put(`http://localhost:4009/movies/${req.params.id}`, {
            title: req.body.title,
            overview: req.body.overview,
            poster_path: req.body.poster_path,
            popularity: req.body.popularity,
    ...
    static delete (req, res) {
        axios.delete(`http://localhost:4009/movies/${req.params.id}`)
            .then( async (allMovie) => {
                const currentMovies = await redis.get('movies')
